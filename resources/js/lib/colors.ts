/**
 * Centralize tag color mapping to ensure consistency across the application.
 * Storing only the color name (e.g., 'blue') in the database allows for
 * flexible theme-aware styling.
 */

export const getTagColorClass = (colorName: string | null) => {
    const name = (colorName || 'slate').toLowerCase();

    const colorMap: Record<string, string> = {
        // Row 1: Neutrals
        slate: 'bg-slate-500/15 text-slate-600 dark:text-slate-400 border-slate-500/20',
        gray: 'bg-gray-500/15 text-gray-600 dark:text-gray-400 border-gray-500/20',
        zinc: 'bg-zinc-500/15 text-zinc-600 dark:text-zinc-400 border-zinc-500/20',
        neutral:
            'bg-neutral-500/15 text-neutral-600 dark:text-neutral-400 border-neutral-500/20',
        stone: 'bg-stone-500/15 text-stone-600 dark:text-stone-400 border-stone-500/20',
        dark: 'bg-zinc-900/15 text-zinc-900 dark:text-zinc-300 border-zinc-900/20',
        ...
        yellow: 'bg-yellow-400/20 text-yellow-700 dark:text-yellow-400 border-yellow-400/20',
        ...
        maroon: 'bg-red-900/15 text-red-900 dark:text-red-400 border-red-900/20',
    };

    return colorMap[name] || colorMap.slate;
};

export const getIconColorClass = (colorName: string | null) => {
    const name = (colorName || 'slate').toLowerCase();
    const colorMap: Record<string, string> = {
        ...
        pink: 'text-pink-500',
        rose: 'text-rose-500',
        maroon: 'text-red-900 dark:text-red-100',
    };
